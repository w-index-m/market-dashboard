...
# ===================================================
# Twitter(X) 言及銘柄トラッカー (@aleabitoreddit)
# ===================================================
_NITTER_INSTANCES = [
    ...
    "https://nitter.twitterno.de",
]
_TICKER_EXCLUDE = {
    "A", "I", "AI", "US", "EU", "UK", "IPO", "ETF", "CEO", "CFO", "CTO",
    "USD", "JPY", "EUR", "GDP", "CPI", "PMI", "EPS", "PE", "PB", "ROE",
    "YOY", "QOQ", "VC", "ML", "NLP", "API", "SaaS", "LLM", "GPU", "CPU",
}

@st.cache_data(ttl=1800, show_spinner=False)
def fetch_twitter_tickers(username: str = "aleabitoreddit") -> Dict[str, Any]:
    """Nitter RSS から$ティッカーを自動抽出（30分キャッシュ）"""
    tweets: List[Dict] = []
    ...
    for instance in _NITTER_INSTANCES:
        try:
            ...
            for entry in feed.entries[:30]:
                ...
                # $TICKER 抽出
                raw_tickers = re.findall(r"\$([A-Z]{1,5})\b", text_clean)
                tickers = list(dict.fromkeys(
                    t for t in raw_tickers if t not in _TICKER_EXCLUDE
                ))
            ...
            logger.debug(f"Nitter {instance} failed: {_e}")
            continue

    from collections import Counter
    all_tickers = [t for tw in tweets for t in tw["tickers"]]
    counter = Counter(all_tickers)

    return {
        "ok":           bool(tweets),
        "source":       source_instance or "",
        "tweets":       tweets,
        "ticker_counts": dict(counter.most_common(30)),
        "all_tickers":  list(counter.keys()),
        "fetched_at":   datetime.now(JST).strftime("%Y-%m-%d %H:%M JST"),
    }


def render_twitter_tracker():
    """@aleabitoreddit 言及銘柄トラッカーを描画"""
    st.header("📡 @aleabitoreddit 言及銘柄トラッカー")
    st.caption("Nitter RSS 経由で自動取得。$ティッカー形式の銘柄を抽出し株価を確認できます。")

    col_btn1, col_btn2 = st.columns([4, 1])
    with col_btn2:
        if st.button("🔄 今すぐ更新", key="twitter_refresh"):
            fetch_twitter_tickers.clear()
            st.rerun()

    with st.spinner("ツイートを取得中..."):
        data = fetch_twitter_tickers()

    if not data["ok"]:
        st.error(
            "❌ Nitter 全インスタンスへの接続に失敗しました。\n\n"
            "Nitter は不安定なため一時的に取得できない場合があります。"
            "しばらくしてから「今すぐ更新」を押してください。"
        )
        return

    st.caption(
        f"✅ {data['source']} から取得 | "
        f"{len(data['tweets'])}件のツイート | "
        f"更新: {data['fetched_at']}"
    )

    # ── 言及頻度ランキング ──────────────────────────────
    if data["ticker_counts"]:
        st.markdown("#### 🏆 直近言及ティッカー（頻度順）")
        tc = data["ticker_counts"]
        cols = st.columns(min(len(tc), 6))
        for i, (ticker, count) in enumerate(list(tc.items())[:12]):
            with cols[i % 6]:
                st.metric(f"${ticker}", f"{count}回")

    # ── ティッカー選択 → 株価・チャート ─────────────────
    st.markdown("#### 📈 株価確認")
    selected = st.multiselect(
        "確認するティッカーを選択（複数可）",
        options=data["all_tickers"],
        default=data["all_tickers"][:4] if data["all_tickers"] else [],
        key="twitter_ticker_select",
    )
            ...
            with price_cols[i % 4]:
                try:
                    ...
                    if price and prev and prev > 0:
                        ...
                        chg_str = f"{chg:+.2f}%"
                        delta_c = "normal" if chg >= 0 else "inverse"
                    else:
                        chg_str, delta_c = "N/A", "off"
                    st.metric(
                        f"${ticker}",
                    ...
                    st.metric(f"${ticker}", "取得失敗")

        # 選択銘柄のチャート
        if len(selected) == 1:
            try:
                hist = yf.Ticker(selected[0]).history(period="3mo", interval="1d")
                if not hist.empty and PLOTLY_AVAILABLE:
                    import plotly.graph_objects as go
                    fig = go.Figure(go.Scatter(
                        x=hist.index, y=hist["Close"],
                        mode="lines", name=selected[0],
    ...
    # ── 最新ツイート一覧 ─────────────────────────────────
    st.markdown("#### 📋 最新ツイート（ティッカー言及あり）")
    shown = 0
    for tw in data["tweets"]:
        if not tw["tickers"]:
            continue
        tickers_md = "  ".join(f"`${t}`" for t in tw["tickers"])
        with st.expander(f"{tickers_md} — {tw['title'][:60]}...", expanded=False):
            st.markdown(tw["text"])
            if tw["link"]:
                st.markdown(f"[X で見る]({tw['link']})")
            if tw["published"]:
                st.caption(tw["published"])
        shown += 1
        if shown >= 15:
            break

    if shown == 0:
        st.info("ティッカー言及のあるツイートが見つかりませんでした。")
...
3. **移動平均乖離率（20%）**: 50日移動平均との差
    ...
    # @aleabitoreddit 言及銘柄トラッカー
    render_twitter_tracker()
    st.divider()
